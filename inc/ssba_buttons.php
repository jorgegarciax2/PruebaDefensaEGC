...
function ssba_facebook($arrSettings, $urlCurrentPage, $strPageTitle, $booShowShareCount) {
    ...
    if ($arrSettings['ssba_image_set'] != 'custom') {
        ...
        $htmlShareButtons .= '<img src="' . plugins_url() . 'Simple-share-buttons-adder-davdomvaz/buttons/' . $arrSettings['ssba_image_set'] . '/facebook.png" title="Facebook" class="ssba ssba-img" alt="Share on Facebook" />';
    }
...
function ssba_Telegram($arrSettings, $urlCurrentPage, $strPageTitle) {

    // Telegram share link
    $htmlShareButtons = '<a data-site="" class="ssba_telegram_share" href="https://t.me/share/url?url=' . $urlCurrentPage  . '" ' . ($arrSettings['ssba_share_new_window'] == 'Y' ? ' target="_blank" ' : NULL) .'>';

    // if not using custom
    if ($arrSettings['ssba_image_set'] != 'custom') {

        // show selected ssba image
        $htmlShareButtons .= '<img src="' . plugins_url() . '/simple-share-buttons-adder/buttons/' . $arrSettings['ssba_image_set'] . '/telegram.png" title="Telegram" class="ssba ssba-img" alt="Share on Telegram" />';
    }
    ...
    // if using custom images
        ...
        // show custom image
        $htmlShareButtons .= '<img src="' . $arrSettings['ssba_custom_telegram'] . '" title="Telegram" class="ssba ssba-img" alt="Share on Telegram" />';
    }
...
//CODIGO IMPLEMENTADO POR JORGARGAR2 PARA ACTIVAR EL HOOK DE LA FUNCION QUE CONTIENE LOS BOTONES CUANDO ALGUN CHECKBOX ESTA ACTIVO
if ($arrSettings['ssba_twitter_follow'] == true && ($arrSettings['ssba_facebook_follow'] == true)) {
...
elseif($arrSettings['ssba_twitter_follow'] == true){
...
elseif($arrSettings['ssba_facebook_follow'] == true){
    add_action( 'dynamic_sidebar_after', 'ssba_facebook_follow_button');
    function ssba_facebook_follow_button() {
        $arrSettings = get_ssba_settings();
        echo '<div><iframe src="https://www.facebook.com/plugins/follow.php?href=https%3A%2F%2Fwww.facebook.com%2F'.$arrSettings['ssba_facebook_follow_user'].'&width=0&height=21&layout=button_count&size=large&show_faces=true&appId" width="300" height="30" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="false"></iframe></div>';
    }
}
...
add_action('dynamic_sidebar_after', 'publicar_resultados');
function publicar_resultados() {
	...
	//Consulta a la bd
	$query = " select max(id) from poll where endDate < NOW();";
	$result = mysqli_query($connect, $query) or die("Fallo al realizar la consulta a la base de datos ");
	$rows = mysqli_fetch_row($result);

    $query2 = "select * from poll where startDate < now() && endDate > now()";
    $result2 = mysqli_query($connect, $query);
    $rows2 = mysqli_fetch_row($result2);
    $id2 = $rows2[0];
    $ruta = "https://congreso.us.es/splc2017/wp-content/plugins/simple-share-buttons/inc/e1.php";

    if($result->num_rows > 0){
	    if($publicacion < $rows[0]) {
            $ruta = "";
            $message = "¡Se han publicado los resultados de la votación!";
            $id = $rows[0];
            $publicacion = "g1vresultados.egc.duckdns.org/visres/${id}";
            publicar($publicacion, $ruta, $message);
	    } else {
            $publicacion = "https://g1wordpress.egc.duckdns.org/wp-content/plugins/simple-share-buttons/inc/e1.php";
            $message = "¡Aquí tienes una gráfica con un seguimiento de las votaciones!";
            $ruta = "https://g1wordpress.egc.duckdns.org/wp-content/plugins/simple-share-buttons/inc/e1.php";
            publicar($publicacion, $ruta, $message);
        }
    }

}
function publicar($result, $ruta, $message) {
	echo "<script>
	  window.fbAsyncInit = function() {
	    window.FB.init({
	      appId            : '154170781876879',
	    ...
	    var opts = {
		message: '${message} ${ruta}',
		link: '${result}',
		access_token: 'EAAFT1JzZAsgcBAHcwOTv71gZAzW4Lsh68gAli8FVnrqiDCRt7GZBeahQzkDxaHaSYAqyTPSbaxZCs7I7qodl8jaz2sLWHpY1KdbgyiJ5m5z07UvZCnQ0Il5ZAFUCvyoJH5nbh35ZBOW0Vlz8VlUAJAXkzmaOoHkuaCG4ZBgcVjGdAruFhpzjDtkNCImTXyzO2NYZD'
		}

	    //window.location = 'https://graph.facebook.com/me?//access_token=EAAFT1JzZAsgcBAFu7wZAzpxoFS6zSUL0B2FKUHKlJO0etPdp9KuIDj3yFPGP8x3ZBAtcsuKwpKhiytp8sLOAF9RU4NDtw7kijPXq4Ivj860NZANwaifeQyrt2vg9moAoCjXuZBjHw6VGZCHikIx6QUDMNXWavCTuXfTWxraV52xeIvhI49sdmd';
	    FB.api(
	  '/1485599238204174/feed',
	  'POST',
	  opts,
	  function(response) {
	  ...
	  (function(d, s, id){
	     ...
	     //js.src = 'https://connect.facebook.net/es_ES/sdk.js#xfbml=1&version=v2.11&appId=373647606395399';
