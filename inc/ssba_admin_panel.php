		...
		$htmlShareButtonsForm .= '<div class="tab-pane fade active in" id="core">';
			...
			$htmlShareButtonsForm .= '<div class="col-sm-12">';
				...
				$locs = array(
                    ...
                    //CODIGO IMPLEMENTADO POR JORGARGAR2 PARA AÑADIR LOS CHECKBOX DE ACTIVACIÓN/DESACTIVACIÓN DE LOS BOTONES DE FOLLOW
					'Twitter Follow Button' => array(
						'value' => 'ssba_twitter_follow',
						'checked' => ($arrSettings['ssba_twitter_follow'] == 'Y'  ? true : false)
                    ),
					'Facebook Follow Button' => array(
						'value' => 'ssba_facebook_follow',
						'checked' => ($arrSettings['ssba_facebook_follow'] == 'Y'  ? true : false)
                    ),
                    //CODIGO IMPLEMENTADO POR EUSGARPLA PARA AÑADIR LOS CHECKBOXES DE ACTIVACION Y DESACTIVACION DEL FEED DE TWITTER Y FACEBOOK
                    'Facebook Page' => array(
                        'value' => 'ssba_facebook_feed',
                        'checked' => ($arrSettings['ssba_facebook_feed'] == 'Y'  ? true : false)
                    ),
                    'Twitter Feed' => array(
                        'value' => 'ssba_twitter_feed',
                        'checked' => ($arrSettings['ssba_twitter_feed'] == 'Y'  ? true : false)
		    )
                ...
                //CODIGO IMPLEMENTADO POR JORGARGAR2 PARA PERMITIR MODIFICAR EL USUARIO DE TWITTER A SEGUIR POR EL BOTON DE FOLLOW
	            $opts = array(
	                'form_group'    => false,
	                'type'          => 'text',
	                'placeholder'	=> 'For example(Not include nickname with @): congresosplcus ',
	                'name'          => 'ssba_twitter_follow_user',
	                'label'        	=> 'Twitter Nickname to follow',
	                'tooltip'       => 'Add nickname which will appear in Twitter follow button',
	                'value'         => $arrSettings['ssba_twitter_follow_user'],
	            );
                $htmlShareButtonsForm .= $ssbpForm->ssbp_input($opts);
                
                //CODIGO IMPLEMENTADO POR JORGARGAR2 PARA PERMITIR MODIFICAR LA PAGINA DE FACEBOOK A SEGUIR POR EL BOTON DE FOLLOW
	            $opts = array(
	                'form_group'    => false,
	                'type'          => 'text',
	                'placeholder'	=> 'For example(Facebook Page Id): Splc-2017-1485599238204174',
	                'name'          => 'ssba_facebook_follow_user',
	                'label'        	=> 'Facebook Page to follow',
	                'tooltip'       => 'Add Facebook Page Id which will appear in Facebook follow button',
	                'value'         => $arrSettings['ssba_facebook_follow_user'],
	            );
				$htmlShareButtonsForm .= $ssbpForm->ssbp_input($opts);
                              
                
                //CODIGO IMPLEMENTADO POR EUSGARPLA PARA PERMITIR MODIFICAR LA PAGINA DE FACEBOOK
	            $opts = array(
	                'form_group'    => false,
	                'type'          => 'text',
	                'placeholder'	=> 'For example(Facebook Page Id): Splc-2017-1485599238204174 - YOU NEED TO SET A FACEBOOK PAGE ID TO MAKE IT FUNCTIONAL',
	                'name'          => 'ssba_facebook_page_name',
	                'label'        	=> 'Facebook Page',
	                'tooltip'       => 'Add Facebook Page Id which will appear in Facebook feed',
	                'value'         => $arrSettings['ssba_facebook_page_name'],
	            );
				$htmlShareButtonsForm .= $ssbpForm->ssbp_input($opts);
